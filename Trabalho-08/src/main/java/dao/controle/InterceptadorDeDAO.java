...
public class InterceptadorDeDAO implements MethodInterceptor {
	...
	if (metodo.isAnnotationPresent(RecuperaLista.class)) { // O método buscaLista() retorna um List
	    return daoGenerico.buscaLista(metodo, args);
	}
	else if (metodo.isAnnotationPresent(RecuperaConjunto.class)) { // O método buscaConjunto() retorna um Set
	    return daoGenerico.buscaConjunto(metodo, args);
	}
	else if (metodo.isAnnotationPresent(RecuperaUltimoOuPrimeiro.class)) { // O método buscaUltimoOuPrimeiro()
									       // retorna um Objeto (Entidade)
	    return daoGenerico.buscaUltimoOuPrimeiro(metodo, args);
	}
	else if (metodo.isAnnotationPresent(RecuperaObjeto.class)) { // O método busca() retorna um Objeto (Entidade)
	    return daoGenerico.busca(metodo, args);
	}
	else if(metodo.getName().equals("hashCode")) {
		System.out.println("===>>>>>>>>>> Executou hashCode()");
		return metodoDoProxy.invokeSuper(objeto, args);
	}
	else {
	    throw new InfraestruturaException("O método " +metodo.getName()+ " nao foi anotado");
	    // return metodoDoProxy.invokeSuper(objeto, args);
	}
    }
}
